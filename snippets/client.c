...
client_connect(void) {
    ...
    if ((res = lwcell_conn_start(NULL, LWCELL_CONN_TYPE_TCP, "example.com", 80, NULL, conn_callback_func, 0)) == lwcellOK) {
...
conn_callback_func(lwcell_evt_t* evt) {
    ...
    switch (lwcell_evt_get_type(evt)) {
        case LWCELL_EVT_CONN_ACTIVE: {             /* Connection just active */
            ...
            res = lwcell_conn_send(conn, req_data, sizeof(req_data) - 1, NULL, 0); /* Start sending data in non-blocking mode */
            if (res == lwcellOK) {
