 ...
 *
 * When initializing low-level part, following steps are important and must be done when \ref gsm_ll_init function is called:
 *
 ...
 *
 * \note            The best case to implement this part is in \ref GSM_LL module file
 *
 ...
 *
 * The size of received buffer is set with \ref GSM_CFG_RCV_BUFF_SIZE configuration parameter.
 ...
 * in memory manager. Take a look at \ref GSM_MEM section for more information
 *
 ...
 * reads the received data from AT port and calls \ref gsm_input_process function directly.
 * 
 ...
 *
 * \note            To use this mode, \ref GSM_CFG_INPUT_USE_PROCESS must be enabled.
