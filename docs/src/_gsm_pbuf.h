 ...
 *
 * When we are dealing with application part, it is important to know what is the difference between \ref gsm_pbuf_cat and \ref gsm_pbuf_chain.
 *
 ...
 *
 * After we call \ref gsm_pbuf_cat, we get a new structure which is on image below.
 *
 * \image html pbuf_cat_vs_chain_2.svg Pbufs structure after calling gsm_pbuf_cat
 * 
 * We can see that reference of second is still set to `1`, but `2` variables are pointing to this block.
 * After we call \ref gsm_pbuf_cat, we have to forget using user variable `2`, because if we somehow try to free pbuf from variable `1`, 
 * then variable `2` points to undefined memory.
 *
 * \include         _example_pbuf_cat.c
 *
 * If we need to link pbuf to another pbuf and we still need to use variable, then use \ref gsm_pbuf_chain instead.
 *
 * \image html pbuf_cat_vs_chain_3.svg Pbufs structure after calling gsm_pbuf_chain
 *
 ...
 *
 * Once we are done using pbuf, we have to free it using \ref gsm_pbuf_free function.
 *
