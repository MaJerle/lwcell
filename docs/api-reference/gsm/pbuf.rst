...
When application tries to free pbuf chain as on first image, it would normally call :cpp:func:`gsm_pbuf_free` function. That would:
...
.. note::
	...
	*User variable 2* is still using *pbuf* starting at *block 2* and must manually call :cpp:func:`gsm_pbuf_free` to free it.
...
.. note::
	After application calls :cpp:func:`gsm_pbuf_cat`, it must not use pointer which points to *second pbuf*.
...
.. note::
	After application calls :cpp:func:`gsm_pbuf_chain`,
	it also has to manually free its reference using :cpp:func:`gsm_pbuf_free` function.
